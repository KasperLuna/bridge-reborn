...
const KickDialog = ({ onClose }: KickDialogProps) => {
  ...
  const bots = seats.filter((s) => !s.is_spectator && s.seat && s.is_bot);
  ...
  const openVote = kickVotes.find((v) => v.status === "open") ?? null;
  const currentVote =
      ...
      >
        ...
        {openVote ? (
          <>
            ...
            {(() => {
              const voteState: "voted" | "can-vote" | "target" | "spectator" =
                alreadyVoted
                  ? "voted"
                  : canVote
                    ? "can-vote"
